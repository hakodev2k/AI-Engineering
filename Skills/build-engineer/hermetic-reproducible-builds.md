...
Make identical declared inputs produce equivalent artifacts independent of machine-local state. This skill is intended for Senior Build Engineers working across repositories and build ecosystems.
...
1. Capture toolchains, environment, clocks, locale, paths, network reads, and generated metadata.
2. Classify inputs as required, accidental, or forbidden.
3. Pin required versions and integrity data.
4. Remove ambient reads from profiles, global SDKs, mutable registries, and current time.
5. Normalize timestamps, locale, path prefixes, and archive ordering.
6. Run isolated builds with network disabled after declared fetch.
7. Compare hashes or normalized artifact contents across independent builds.

## Decision points

- Require bit-for-bit identity for release artifacts when feasible; otherwise document normalized equivalence.
- Vendor or mirror critical inputs when external availability threatens reproducibility.
