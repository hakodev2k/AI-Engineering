...
Instrument builds so latency, failures, cache behavior, dependency changes, and resource bottlenecks are diagnosable over time. This skill is intended for Senior Build Engineers working across repositories and build ecosystems.
...
1. Define SLIs for wall time, queue time, success, flakes, cache hits, critical path, and failure classes.
2. Emit structured build metadata with revision, platform, toolchain, and mode.
3. Capture useful target timings and graph context.
4. Correlate runner metrics with build events.
5. Build trends without leaking sensitive args or environment data.
6. Set regression thresholds and owners.
7. Retain representative traces.
8. Control telemetry cost/cardinality.

## Decision points

- Collect enough target detail for diagnosis without unbounded labels.
- Alert on sustained or high-impact regression rather than noise.
