...
Test build logic itself so target graphs, incremental behavior, packaging, configuration, and release guards have regression coverage. This skill is intended for Senior Build Engineers working across repositories and build ecosystems.
...
1. Identify high-risk build behaviors.
2. Create minimal fixture projects.
3. Test clean and incremental scenarios separately.
4. Add negative cases for missing inputs, incompatible versions, forbidden deps, and unauthorized publication.
5. Normalize nondeterministic golden outputs.
6. Run build-system tests before broad project builds.
7. Preserve regressions as focused fixtures.
8. Keep tests fast enough for relevant CI changes.

## Decision points

- Prefer behavioral integration tests for process orchestration.
- Use unit tests for pure graph/config transformations.
