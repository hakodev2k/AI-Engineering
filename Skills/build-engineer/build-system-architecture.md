...
Design and evolve explicit build graphs, target boundaries, toolchain isolation, and local/CI parity. This skill is intended for Senior Build Engineers working across repositories and build ecosystems.
...
1. Inventory languages, package managers, generators, targets, artifacts, and entry points.
2. Model explicit dependencies and identify hidden filesystem, environment, network, and ordering inputs.
3. Define stable target boundaries aligned with reusable or deployable units.
4. Separate restore, generation, compilation, testing, packaging, and publishing.
5. Make inputs, outputs, tool versions, and side effects explicit.
6. Measure critical path, fan-out, cacheability, and failure concentration.
7. Roll out structural changes incrementally with rollback.

## Decision points

- Prefer simple native tooling at modest scale; use graph-oriented systems when incremental correctness and monorepo scale justify complexity.
- Centralize policy while keeping target-specific logic close to owners.
