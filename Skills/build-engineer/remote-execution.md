...
Use remote execution when distributed hermetic actions materially improve build critical paths. This skill is intended for Senior Build Engineers working across repositories and build ecosystems.
...
1. Confirm candidate actions are hermetic.
2. Measure whether remote parallelism exceeds transfer/queue overhead.
3. Define OS, architecture, toolchain, and resource platform properties.
4. Minimize content-addressed input trees.
5. Start with deterministic expensive actions.
6. Instrument queue, transfer, execution, cache, retry, and failure rates.
7. Provide local fallback when appropriate.
8. Expand only after correctness and economics are proven.

## Decision points

- Use remote execution for expensive parallelizable actions; tiny low-latency tasks may remain local.
- Measure remote cache and remote execution benefits separately.
