...
Scale build graphs, affected analysis, caching, CI, and ownership for large repositories. This skill is intended for Senior Build Engineers working across repositories and build ecosystems.
...
1. Construct the repository dependency graph.
2. Classify compile, test, runtime, generation, and packaging edges.
3. Detect cycles and boundary violations.
4. Define affected-target calculation from ownership and dependency closure.
5. Centralize common policy while retaining team-owned target config.
6. Adopt remote cache/execution only after hermeticity.
7. Shard graph work safely.
8. Track graph growth, affected counts, cache hits, and critical path.

## Decision points

- Prefer explicit project metadata over path heuristics.
- Break cycles before adding more parallelism.
