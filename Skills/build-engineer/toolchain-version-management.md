...
Pin, provision, upgrade, and audit compilers, SDKs, runtimes, package managers, and build utilities. This skill is intended for Senior Build Engineers working across repositories and build ecosystems.
...
1. Inventory tools affecting outputs.
2. Identify current version-selection mechanisms.
3. Create one authoritative declaration per toolchain boundary.
4. Automate provisioning or fail-fast validation.
5. Record compatibility constraints.
6. Test upgrades across clean, incremental, test, package, and reproducibility scenarios.
7. Roll upgrades through CI first.
8. Remove obsolete versions.

## Decision points

- Prefer repository-declared versions over machine-global defaults.
- Use containers only when they reduce real environment variance rather than hiding poor version management.
