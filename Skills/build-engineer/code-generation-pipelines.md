...
Integrate generators so outputs are deterministic, correctly invalidated, and safe to consume. This skill is intended for Senior Build Engineers working across repositories and build ecosystems.
...
1. Identify authoritative inputs and output consumers.
2. Choose checked-in versus ephemeral generation based on bootstrap, tooling, review, and churn needs.
3. Pin generator, templates, and plugins.
4. Declare generation targets with explicit inputs/outputs.
5. Remove machine paths, clocks, and hidden network reads.
6. Add stale-output verification for checked-in artifacts.
7. Test schema-breaking changes.
8. Separate generated code from handwritten extension points.

## Decision points

- Check in outputs when consumers cannot reliably regenerate or API review is important; otherwise prefer generated build outputs.
