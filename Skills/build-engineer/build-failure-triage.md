...
Classify build failures quickly and isolate the earliest actionable cause rather than cascades. This skill is intended for Senior Build Engineers working across repositories and build ecosystems.
...
1. Capture revision, command, environment, versions, and complete logs.
2. Find the first causal error.
3. Classify as compile, resolution, generation, linking, packaging, environment, cache, infrastructure, or permission failure.
4. Reproduce the narrowest target and disable cache if implicated.
5. Compare known-good revision/environment.
6. Inspect relevant recent source, lockfile, toolchain, build-script, and runner changes.
7. Apply the smallest evidence-backed fix.
8. Add regression coverage or better diagnostics.

## Decision points

- Use bisection when the failing range is unclear but reproducible.
- Escalate runner/network failures with infrastructure evidence instead of changing build logic.
