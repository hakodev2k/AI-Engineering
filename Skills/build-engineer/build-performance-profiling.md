...
Reduce build latency using repeatable measurements and critical-path analysis. This skill is intended for Senior Build Engineers working across repositories and build ecosystems.
...
1. Benchmark cold, warm, clean, and incremental builds.
2. Capture target timings and CPU, memory, disk, and network utilization.
3. Identify the critical path.
4. Classify bottlenecks as compute, serialization, restore, I/O, network, cache, startup, or scheduler related.
5. Form one optimization hypothesis at a time.
6. Apply low-risk graph, cache, dependency, or tool improvements.
7. Re-run identical benchmarks.
8. Protect improvements with budgets or regression alerts.

## Decision points

- Optimize critical-path work before off-path aggregate CPU.
- Do not trade correctness or reproducibility for marginal speed without explicit acceptance.
