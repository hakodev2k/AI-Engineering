...
Keep dependency graphs intentional, repeatable, auditable, and resistant to transitive drift. This skill is intended for Senior Build Engineers working across repositories and build ecosystems.
...
1. Identify each package-manager resolution boundary.
2. Inspect manifests, lockfiles, registries, overrides, and automation.
3. Require lockfiles or equivalent snapshots where supported.
4. Detect duplicate majors, incompatible constraints, and transitive hotspots.
5. Pin integrity metadata and approved sources.
6. Define upgrade workflows by risk level.
7. Validate graph changes with build, tests, security checks, and artifact checks.

## Decision points

- Use exact pins for toolchains/build-critical utilities; application libraries may use ranges only with locked resolutions.
- Prefer upstream-compatible convergence over permanent forced overrides.
