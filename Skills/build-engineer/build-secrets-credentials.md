# Build Secrets and Credentials

## Purpose

Prevent credential leakage or overexposure during restore, build, signing, testing, and publication. This skill is intended for Senior Build Engineers working across repositories and build ecosystems.
...
1. Inventory all build credentials.
2. Replace long-lived secrets with workload identity or short-lived tokens where possible.
3. Scope credentials by operation, environment, namespace, and lifetime.
4. Keep secrets out of CLI args, caches, artifacts, generated files, and logs.
5. Separate untrusted builds from credentialed publish stages.
6. Clean temporary credential material.
7. Add secret scanning and focused log review.
8. Rotate after suspected disclosure.

## Decision points

- Prefer federated identity over static repository secrets.
- Never expose publish credentials to untrusted pull requests.
