...
Reduce compromise risk across builders, plugins, registries, dependencies, and release artifacts. This skill is intended for Senior Build Engineers working across repositories and build ecosystems.
...
1. Map trust boundaries from checkout to publish.
2. Enumerate executable build inputs including plugins, scripts, generators, actions, images, and package hooks.
3. Pin versions and integrity identifiers.
4. Restrict package sources and namespace mappings.
5. Sandbox untrusted code with minimal filesystem/network/credential access.
6. Protect release workflows and reviewed source state.
7. Generate and verify provenance, SBOM, signatures, and digests.
8. Exercise compromised-builder/dependency incident scenarios.

## Decision points

- Treat build plugins as code execution, not configuration.
- Prefer digest pinning for release-critical images/actions.
