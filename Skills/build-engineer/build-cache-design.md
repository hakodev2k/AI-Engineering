...
Accelerate builds without reusing stale, unsafe, or incorrectly keyed outputs. This skill is intended for Senior Build Engineers working across repositories and build ecosystems.
...
1. Measure expensive cacheable targets.
2. Enumerate semantic inputs including toolchain, flags, platform, environment, generated inputs, and dependency outputs.
3. Construct stable content-based keys.
4. Ensure outputs are immutable and restorable into clean workspaces.
5. Define local/remote scopes, trust boundaries, auth, retention, and eviction.
6. Instrument hit, miss, corruption, upload/download, and bypass rates.
7. Mutation-test invalidation.
8. Provide purge and bypass procedures.

## Decision points

- Do not cache targets with undeclared side effects.
- Use read-only shared-cache access for untrusted builds when poisoning is possible.
