# Build Engineering Code Review

## Purpose

Review build changes for graph correctness, reproducibility, portability, cache safety, security, performance, and release impact. This skill is intended for Senior Build Engineers working across repositories and build ecosystems.
...
1. Identify intended behavior and affected targets/platforms.
2. Inspect new inputs, outputs, env reads, network access, and side effects.
3. Check dependency edges and incremental invalidation.
4. Check tool/dependency pinning.
5. Review quoting, paths, errors, and portability.
6. Assess cache safety and secret exposure.
7. Evaluate CI permissions and untrusted contexts.
8. Require verification evidence proportional to blast radius.

## Decision points

- Require stronger evidence for release/security changes than local convenience changes.
- Prefer explicit readable build logic over clever metaprogramming when diagnosis would suffer.
