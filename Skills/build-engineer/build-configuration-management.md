...
Keep build configuration explicit, composable, reviewable, and separate from runtime environment configuration. This skill is intended for Senior Build Engineers working across repositories and build ecosystems.
...
1. Inventory files, env vars, CLI flags, generated config, and CI params.
2. Classify settings as build-semantic, test, packaging, publishing, or runtime.
3. Create a documented precedence model.
4. Fingerprint build-semantic configuration.
5. Replace environment-name conditionals with explicit target properties where possible.
6. Validate incompatible combinations early.
7. Expose effective config diagnostically without secrets.
8. Test important combinations.

## Decision points

- Runtime variability should normally remain runtime configuration.
- Use build variants only when artifact semantics genuinely differ.
