...
export class WasabiClient {
  ...
  private async call<T>(fn: () => Promise<T>, retryable: boolean): Promise<T> {
    let attempt = 0;
    while (true) {
  ...
  async listBuckets() {
    return this.call(() => this.s3.send(new ListBucketsCommand({})), true);
  }

  async bucketLocation(bucket: string) {
    return this.call(() => this.s3.send(new GetBucketLocationCommand({ Bucket: bucket })), true);
  }
  ...
  async createBucket(bucket: string) {
    ...
    return this.call(() => this.s3.send(new CreateBucketCommand(input)), false);
  }

  async deleteBucket(bucket: string) {
    return this.call(() => this.s3.send(new DeleteBucketCommand({ Bucket: bucket })), false);
  }

  async listObjects(bucket: string, prefix?: string, maxKeys = 100, continuationToken?: string) {
    return this.call(() => this.s3.send(new ListObjectsV2Command({ Bucket: bucket, Prefix: prefix, MaxKeys: maxKeys, ContinuationToken: continuationToken })), true);
  }

  async headObject(bucket: string, key: string) {
    return this.call(() => this.s3.send(new HeadObjectCommand({ Bucket: bucket, Key: key })), true);
  }

  async readText(bucket: string, key: string, maxBytes: number) {
    const result = await this.call(() => this.s3.send(new GetObjectCommand({ Bucket: bucket, Key: key, Range: `bytes=0-${maxBytes - 1}` })), true);
  ...
  async putText(bucket: string, key: string, content: string, contentType?: string) {
    return this.call(() => this.s3.send(new PutObjectCommand({ Bucket: bucket, Key: key, Body: content, ContentType: contentType })), false);
  }
  ...
  async copyObject(bucket: string, sourceKey: string, destinationKey: string) {
    ...
    return this.call(() => this.s3.send(new CopyObjectCommand({ Bucket: bucket, CopySource: encoded, Key: destinationKey })), false);
  }

  async deleteObject(bucket: string, key: string, versionId?: string) {
    return this.call(() => this.s3.send(new DeleteObjectCommand({ Bucket: bucket, Key: key, VersionId: versionId })), false);
  }
