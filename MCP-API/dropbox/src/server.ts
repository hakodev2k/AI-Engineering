...
function result(value: unknown) {
  return { content: [{ type: 'text' as const, text: JSON.stringify({ untrusted_provider_data: true, result: value }) }] };
}
...
server.registerTool('dropbox.shared_link.list', {
  ...
  inputSchema: { path: path.optional(), cursor: z.string().min(1).max(8192).optional() }
...
server.registerTool('dropbox.file.revisions.list', {
  ...
  inputSchema: { path: writePath, limit: z.number().int().min(1).max(100).default(20) }
...
server.registerTool('dropbox.folder.create', {
  ...
  inputSchema: { path: writePath, approval_id: approval }
...
server.registerTool('dropbox.file.copy', {
  ...
  inputSchema: { from_path: writePath, to_path: writePath, autorename: z.boolean().default(false), approval_id: approval }
...
server.registerTool('dropbox.shared_link.create', {
  ...
  inputSchema: { path: writePath, audience: z.enum(['public', 'team', 'no_one']).default('public'), approval_id: approval }
...
server.registerTool('dropbox.file.revision.restore', {
  ...
  inputSchema: { path: writePath, rev: z.string().min(1).max(256), approval_id: approval }
...
server.registerTool('dropbox.file.delete', {
  ...
  inputSchema: { path: writePath, parent_rev: z.string().min(1).max(256).optional(), approval_id: approval }
