...
export function createServer(){
 ...
 for(const t of buildTools(client,config.requireWriteApproval)) server.tool(t.name,t.description,t.schema._def.typeName==='ZodObject'?(t.schema as any).shape:{},async(args:any)=>{try{const input=t.schema.parse(args);const out=await t.run(input);return {content:[{type:'text',text:JSON.stringify({ok:true,risk:t.risk,approvalRequired:t.approval,result:out})}]};}catch(e){return {isError:true,content:[{type:'text',text:JSON.stringify({ok:false,error:e instanceof Error?e.message:String(e),risk:t.risk})}]};}});
 return server;
}
