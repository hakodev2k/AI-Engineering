...
export class KintoneClient{
 ...
 async request<T>(method:string,path:string,body?:unknown,signal?:AbortSignal):Promise<T>{
  ...
  for(let attempt=0;;attempt++){
   ...
   try{
    ...
    if((res.status===429||res.status>=500)&&retryable.includes(method)&&attempt<this.c.maxRetries){await sleep(Number.isFinite(retryAfter)?retryAfter*1000:Math.min(500*2**attempt,5000));continue}
    throw new KintoneError(res.status,data.code,data.message??`Kintone HTTP ${res.status}`,Number.isFinite(retryAfter)?retryAfter:undefined);
 ...
 listRecords(app:number,query:string='',fields:string[]=[]){const q=new URLSearchParams({app:String(app),query});fields.forEach(f=>q.append('fields',f));return this.request('GET',`/records.json?${q}`)}
