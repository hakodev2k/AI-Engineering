...
const schemas = {
  ...
  'stytch.member.search': z.object({ organizationIds: z.array(id).min(1).max(100), cursor: z.string().max(2048).optional(), limit: z.number().int().min(1).max(1000).default(100), memberIds: z.array(id).max(100).optional(), emails: z.array(email).max(100).optional(), nameFuzzy: z.string().min(3).max(128).optional() }).strict(),
...
export const toolDefinitions = [
  ...
  def('stytch.member.search', 'Search members within explicit organizations with bounded pagination.', objectJson({ organizationIds: arrStr(1,100), cursor: str(), limit: integer(1,1000), memberIds: arrStr(0,100), emails: arrStr(0,100), nameFuzzy: str(3,128) }, ['organizationIds'])),
...
async function searchMembers(client: StytchClient, a: Record<string, unknown>) {
  ...
  if (a.nameFuzzy) operands.push({ filter_name: 'member_name_fuzzy', filter_value: a.nameFuzzy });
