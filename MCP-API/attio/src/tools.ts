...
export const TOOLS: ToolDef[] = [
  ...
  { name:'attio.task.update', upstream:'update-task', risk:'WRITE', description:'Update task status, deadline, assignee, or linked record.', schema:z.object({ task_id:id, status:z.string().max(100).optional(), deadline:z.string().max(64).nullable().optional(), assignee_id:id.nullable().optional(), linked_record_id:id.nullable().optional(), approvalId }).strict().refine(v => [v.status,v.deadline,v.assignee_id,v.linked_record_id].some(x => x !== undefined), 'At least one update field is required') },
