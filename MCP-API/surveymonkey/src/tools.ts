...
const responsePerPage = z.number().int().min(1).max(100).optional();
...
const surveyUpdate = z.object({surveyId:id,title:z.string().min(1).max(250).optional(),nickname:z.string().max(250).optional(),language:z.string().min(2).max(20).optional(),approvalToken}).strict().refine(v=>v.title!==undefined||v.nickname!==undefined||v.language!==undefined,"At least one update field is required");
...
export const TOOLS: ToolDef[] = [
  ...
  { name:"surveymonkey.response.list", description:"List survey responses with full answer data using the bulk response endpoint.", risk:"READ", permission:"responses_read_detail", approval:false, schema:z.object({surveyId:id,page,perPage:responsePerPage}).strict(), inputSchema:obj({surveyId:stringId,page:{type:"integer",minimum:1},perPage:{type:"integer",minimum:1,maximum:100}},["surveyId"]) },
