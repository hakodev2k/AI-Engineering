...
['adyen.payment.cancel','cancel_payment','Cancel an authorized payment before capture.','HIGH_RISK',z.object({paymentReference:id,merchantAccount:id,approvalToken}).strict()],
['adyen.payment.refund','refund_payment','Refund a captured payment.','HIGH_RISK',z.object({pspReference:id,currency:z.string().length(3).transform(v=>v.toUpperCase()),value:z.number().int().positive(),merchantAccount:id,reference:z.string().min(1).max(80),approvalToken}).strict()],
