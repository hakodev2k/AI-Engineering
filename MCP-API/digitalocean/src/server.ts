...
server.tool('digitalocean.account.get', 'Get the authenticated DigitalOcean account profile.', {}, async () =>
  out(await rest.get('/account'))
);

server.tool('digitalocean.region.list', 'List DigitalOcean regions and availability metadata.', { page, perPage }, async a =>
  out(await rest.get('/regions', { page: a.page, per_page: a.perPage }))
);

server.tool('digitalocean.droplet.list', 'List Droplets. Read-only.', { page, perPage, tagName: z.string().max(255).optional() }, async a =>
  out(await mcp.call('droplets', 'droplet-list', { Page: a.page ?? 1, PerPage: a.perPage ?? 50 }, () =>
    rest.get('/droplets', { page: a.page, per_page: a.perPage, tag_name: a.tagName })))
);
...
server.tool('digitalocean.droplet.create', 'Create a Droplet. Requires explicit approval.', {
  ...
  imageSlug: z.string().min(1).max(255),
  sshKeys: z.array(z.union([z.string().min(1).max(255), z.number().int().positive()])).max(100).optional(),
  tags: z.array(z.string().min(1).max(255)).max(100).optional(),
...
server.tool('digitalocean.droplet.reboot', 'Reboot a Droplet. HIGH_RISK; requires explicit approval.', { id: z.number().int().positive(), approvalId }, async a =>
  dropletAction('digitalocean.droplet.reboot', 'droplet-reboot', a.id, { type: 'reboot', approvalId: a.approvalId })
);
server.tool('digitalocean.droplet.power_on', 'Power on a Droplet. HIGH_RISK; requires explicit approval.', { id: z.number().int().positive(), approvalId }, async a =>
  dropletAction('digitalocean.droplet.power_on', 'power-on-droplet', a.id, { type: 'power_on', approvalId: a.approvalId })
);
server.tool('digitalocean.droplet.power_off', 'Power off a Droplet. HIGH_RISK; requires explicit approval.', { id: z.number().int().positive(), approvalId }, async a =>
  dropletAction('digitalocean.droplet.power_off', 'power-off-droplet', a.id, { type: 'power_off', approvalId: a.approvalId })
);
server.tool('digitalocean.droplet.snapshot', 'Create a Droplet snapshot. Requires explicit approval.', { id: z.number().int().positive(), name: z.string().min(1).max(255), approvalId }, async a =>
  dropletAction('digitalocean.droplet.snapshot', 'snapshot-droplet', a.id, { name: a.name, approvalId: a.approvalId })
);

server.tool('digitalocean.firewall.list', 'List Cloud Firewalls. Read-only.', { page, perPage }, async a =>
  out(await mcp.call('networking', 'firewall-list', { Page: a.page ?? 1, PerPage: a.perPage ?? 20 }, () =>
    rest.get('/firewalls', { page: a.page, per_page: a.perPage })))
);
...
server.tool('digitalocean.firewall.create', 'Create a Cloud Firewall with one inbound and one outbound rule. HIGH_RISK; requires approval.', {
  name: z.string().min(1).max(255),
  inboundProtocol: z.enum(['tcp', 'udp', 'icmp']),
  inboundPortRange: z.string().min(1).max(64),
  inboundSource: z.string().min(1).max(255),
  outboundProtocol: z.enum(['tcp', 'udp', 'icmp']),
  outboundPortRange: z.string().min(1).max(64),
  outboundDestination: z.string().min(1).max(255),
  dropletIds: z.array(z.number().int().positive()).max(100).optional(),
  tags: z.array(z.string().min(1).max(255)).max(100).optional(),
  approvalId
}, async a => {
...
server.tool('digitalocean.firewall.add_droplets', 'Attach Droplets to a Cloud Firewall. HIGH_RISK; requires approval.', {
  ...
  return out(await mcp.call('networking', 'firewall-add-droplets', { ID: a.id, DropletIDs: a.dropletIds }, () =>
    rest.post(`/firewalls/${a.id}/droplets`, { droplet_ids: a.dropletIds })));
});
