...
export class LaunchDarklyMcpClient {
  ...
  async call(tool: string, request: Record<string, unknown>, scope: 'read' | 'write'): Promise<unknown> {
    ...
    const transport = this.config.mcpMode === 'hosted'
      ? this.hostedTransport()
      : this.localTransport(tool, scope);
  ...
  private hostedTransport(): StreamableHTTPClientTransport {
    if (!this.config.mcpAccessToken) {
      throw new Error('Hosted MCP requires a securely provisioned LAUNCHDARKLY_MCP_ACCESS_TOKEN from an OAuth-capable credential broker');
    }
  ...
  private localTransport(tool: string, scope: 'read' | 'write'): StdioClientTransport {
    ...
    return new StdioClientTransport({
      command: 'npx',
      args: ['-y', '--package', '@launchdarkly/mcp-server', '--', 'mcp', 'start', '--api-key', this.config.accessToken, '--scope', scope, '--tool', tool],
