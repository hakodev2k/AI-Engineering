...
export type PolicyConfig = { allowWrite: boolean; approvalMode: "required" | "disabled" };
...
export function assertAllowed(risk: Risk, approval: Approval | undefined, cfg: PolicyConfig): void {
  if (risk === "READ") return;
  if (risk === "DESTRUCTIVE") throw new PolicyError("Destructive Courier operations are disabled by this connector.");
  if (!cfg.allowWrite) throw new PolicyError("Write operations are disabled because COURIER_ALLOW_WRITE=false.");
