...
register('notion.page.create','Create one or more pages. Requires approval.',{parent:z.record(z.unknown()).optional(),pages:z.array(z.record(z.unknown())).min(1).max(20),approvalId:z.string()},'notion-create-pages','WRITE');
register('notion.page.update','Update a page. Requires approval.',{page_id:z.string().min(1),command:z.string().min(1),approvalId:z.string(),properties:z.record(z.unknown()).optional(),content:z.string().max(200000).optional(),new_str:z.string().max(200000).optional()},'notion-update-page','WRITE');
...
register('notion.page.move','Move pages to a new parent. High-risk; requires approval.',{page_or_database_ids:z.array(z.string()).min(1).max(20),new_parent:z.record(z.unknown()),approvalId:z.string()},'notion-move-pages','HIGH_RISK');
...
register('notion.database.create','Create a database. Requires approval.',{parent:z.record(z.unknown()),title:z.string().min(1).max(200),properties:z.record(z.unknown()),approvalId:z.string()},'notion-create-database','WRITE');
