...
export type ToolDef = {
  ...
  upstreamMcp: string;
  rest: (a: any) => { method: string; path: string; query?: Record<string, any>; body?: unknown; retryable?: boolean };
};
...
export const TOOL_DEFS: ToolDef[] = [
  ...
  { name: 'calendly.event_type.update', description: 'Update selected event-type settings.', schema: z.object({ uuid, name: z.string().min(1).max(200).optional(), duration: z.number().int().min(1).max(720).optional(), active: z.boolean().optional(), approval_id: approval }).refine(v => v.name !== undefined || v.duration !== undefined || v.active !== undefined, 'At least one mutable field is required'), upstreamMcp: 'event_types-update_event_type', rest: a => ({ method: 'PATCH', path: `/event_types/${encodeURIComponent(a.uuid)}`, body: withoutApproval({ name: a.name, duration: a.duration, active: a.active }), retryable: false }) }
];
...
function withoutApproval(input: Record<string, unknown>) {
  return Object.fromEntries(Object.entries(input).filter(([k, v]) => k !== 'approval_id' && v !== undefined));
}
