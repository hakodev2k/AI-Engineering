...
const server = new Server({ name:"tavily-connector", version:"1.0.0" }, { capabilities:{ tools:{} } });
...
server.setRequestHandler(CallToolRequestSchema, async request=>{
  try {
    ...
    return { isError:true, content:[{ type:"text", text:JSON.stringify({ ok:false, error:{ name:error?.constructor?.name || "Error", message:error instanceof Error ? error.message : "Unknown connector error" } }) }] };
  }
});
