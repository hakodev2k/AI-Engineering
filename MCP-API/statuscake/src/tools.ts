...
export function registerTools(server: McpServer, api: StatusCakeClient, config: StatusCakeConfig): void {
  ...
  register(server, config, "statuscake.uptime.history", "Read execution history for an uptime check.", "READ", { testId: id, limit, before: z.number().int().nonnegative().optional(), after: z.number().int().nonnegative().optional() }, a => api.request(`/uptime/${encodeURIComponent(a.testId)}/history`, { query: a }));
  register(server, config, "statuscake.uptime.alerts", "Read alert history for an uptime check.", "READ", { testId: id, limit, before: z.number().int().nonnegative().optional(), after: z.number().int().nonnegative().optional() }, a => api.request(`/uptime/${encodeURIComponent(a.testId)}/alerts`, { query: a }));
  ...
  register(server, config, "statuscake.uptime.update", "Update selected uptime monitor settings.", "WRITE", { testId: id, name: z.string().min(1).max(255).optional(), website_url: z.string().min(1).max(2048).optional(), check_rate: z.union([z.literal(0),z.literal(30),z.literal(60),z.literal(300),z.literal(900),z.literal(1800),z.literal(3600),z.literal(86400)]).optional(), confirmation: z.number().int().min(0).max(3).optional(), paused: z.boolean().optional(), approved }, a => api.request(`/uptime/${encodeURIComponent(a.testId)}`, { method: "PUT", form: a }));
