...
export function approved(tool:string, env:NodeJS.ProcessEnv=process.env){
  if (TOOL_RISK[tool] === 'READ') return true;
  return new Set((env.UPLOADTHING_APPROVED_TOOLS ?? '').split(',').map(x=>x.trim()).filter(Boolean)).has(tool);
}
export function requireApproval(tool:string, env:NodeJS.ProcessEnv=process.env){
  if (!approved(tool,env)) throw new Error(`APPROVAL_REQUIRED: ${tool}`);
}
export function requireToken(env:NodeJS.ProcessEnv=process.env){
  const token=env.UPLOADTHING_TOKEN?.trim(); if(!token) throw new Error('AUTH_CONFIGURATION_ERROR: UPLOADTHING_TOKEN is required'); return token;
}
...
export function createServer(env:NodeJS.ProcessEnv=process.env, client?:UTApi){
  const api=client ?? new UTApi({token:requireToken(env)});
  const server=new McpServer({name:'uploadthing-connector',version:'1.0.0'});
  ...
  server.tool('uploadthing.file.upload_url','Upload one remote HTTPS resource through UploadThing. WRITE; approval required.',{url:z.string().url().refine(v=>v.startsWith('https://'),'HTTPS only'),name:z.string().min(1).max(255).optional(),customId:z.string().min(1).max(255).optional()},async({url,name,customId})=>{requireApproval('uploadthing.file.upload_url',env);return run(()=>api.uploadFilesFromUrl(new URL(url),{metadata:{source:'mcp'},contentDisposition:'inline',acl:'public-read'}).then(r=>({result:r,name,customId})));});
