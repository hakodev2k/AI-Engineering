...
export const tools = [
  ...
  ['temporal_cloud.namespace.tags.update','Add/update/remove namespace tags',Risk.WRITE,z.object({namespace:id,tagsToUpsert:z.record(z.string().max(128),z.string().max(256)).optional(),tagsToRemove:z.array(z.string().min(1).max(128)).max(50).optional(),...approved}).strict().refine(v=>Object.keys(v.tagsToUpsert||{}).length>0||(v.tagsToRemove||[]).length>0,'At least one tag update is required'), (c,a)=>c.request('POST',`/cloud/namespaces/${encodeURIComponent(a.namespace)}/update-tags`,{body:{tagsToUpsert:a.tagsToUpsert,tagsToRemove:a.tagsToRemove},retryable:false})],
...
function zodToJsonSchema(schema){
  const def=schema._def; if(def.typeName!=='ZodObject' && def.type!=='object') return {type:'object',additionalProperties:false};
