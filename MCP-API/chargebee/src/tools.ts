...
const defs:[string,string,Risk,z.ZodTypeAny][] = [
 ...
 ["chargebee.subscription.resume","Resume a paused subscription after explicit approval.","HIGH_RISK",z.object({subscriptionId:id,resumeOption:z.enum(["immediately","scheduled"]).default("immediately"),approvalToken}).strict()],
...
function jsonSchema(schema:z.ZodTypeAny){
 const shape=(schema as any)._def.shape(); const properties:Record<string,unknown>={}; const required:string[]=[];
 for(const [k,v] of Object.entries<any>(shape)){ const d=v._def; const type=d.typeName?.includes("Number")?"number":d.typeName?.includes("Boolean")?"boolean":"string"; properties[k]={type}; if(!v.isOptional()) required.push(k); }
