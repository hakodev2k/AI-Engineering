...
export function assertAllowed(risk: Risk, approval: Approval | undefined, cfg: PolicyConfig): void {
  if (risk === "READ") return;
  if (cfg.readOnly) throw new PolicyError("Write operations are disabled because AIVEN_READ_ONLY=true.");
  if (!cfg.allowWrite) throw new PolicyError("Write operations are disabled because AIVEN_ALLOW_WRITE=false.");
  if (risk === "DESTRUCTIVE") throw new PolicyError("Destructive operations are disabled by this connector.");
  if (cfg.approvalMode === "required") {
    if (!approval?.confirmed || !approval.reason?.trim()) {
