...
export type PolicyConfig = { readOnly: boolean; allowWrite: boolean; approvalMode: "required" | "disabled" };
...
export function assertAllowed(risk: Risk, approval: Approval | undefined, cfg: PolicyConfig): void {
  ...
  if (cfg.readOnly) throw new PolicyError("Write operations are disabled because CLERK_READ_ONLY=true.");
  if (!cfg.allowWrite) throw new PolicyError("Write operations are disabled because CLERK_ALLOW_WRITE=false.");
