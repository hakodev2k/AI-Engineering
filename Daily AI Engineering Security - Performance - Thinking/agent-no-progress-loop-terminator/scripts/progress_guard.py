...
def canonical_signature(tool:str,args:Any)->str:
    raw=json.dumps({"tool":tool,"args":args},sort_keys=True,separators=(",",":"),ensure_ascii=False).encode("utf-8")
    return hashlib.sha256(raw).hexdigest()

def load_events(path:Path)->list[dict[str,Any]]:
    try: lines=path.read_text(encoding="utf-8").splitlines()
    except OSError as exc: raise ValueError(f"cannot read events: {exc}") from exc
    rows=[]
...
def analyze(events:list[dict[str,Any]],policy:dict[str,Any])->dict[str,Any]:
    ...
    sig_counts={}; err_counts={}; no_progress=0; decision="continue"; reason="progress_within_bounds"; evidence=[]
    for row in events:
        ...
        evidence.append({"step":row["step"],"signature":sig[:12],"status":row["status"],"progress":row["progress"],"error_class":err}); evidence=evidence[-5:]
        if row["step"]>=max_steps: decision="terminate"; reason="max_total_steps"; break
        if err:
            count=err_counts[f"{sig}:{err}"]
            if err in trans and count>transient_limit: decision="recover"; reason="transient_retry_budget_exhausted"; break
            if err not in trans and count>=err_limit: decision="recover"; reason="repeated_non_transient_error"; break
        if sig_counts[sig]>=repeat_limit and not row["progress"]: decision="terminate"; reason="repeated_equivalent_action"; break
        if no_progress>=no_prog_limit: decision="terminate"; reason="no_progress_budget_exhausted"; break
...
def main()->int:
    ...
    print(json.dumps(report,indent=2,ensure_ascii=False)); return 3 if a.strict and report["decision"]!="continue" else 0
