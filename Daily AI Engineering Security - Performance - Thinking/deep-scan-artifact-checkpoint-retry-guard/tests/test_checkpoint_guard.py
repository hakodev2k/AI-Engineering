...
class GuardTests(unittest.TestCase):
    ...
    def test_missing_blocks_checkpoint(self):
        with tempfile.TemporaryDirectory() as d:
            cp,p=self.run_guard("checkpoint","--root",d,"--scan-id","s","--revision","r","--phase","discovery","--required","candidate.json")
    ...
    def test_full_retry_requires_approval(self):
        cp,p=self.run_guard("retry","--scope","full","--terminal-failure","--quota-remaining","80")
        self.assertEqual(cp.returncode,3); self.assertEqual(p["status"],"blocked")

    def test_low_quota_blocks_even_approved(self):
        cp,p=self.run_guard("retry","--scope","full","--terminal-failure","--approved","--quota-remaining","5","--min-quota","10")
    ...
    def test_repeated_failure_blocks(self):
        cp,p=self.run_guard("retry","--scope","worker","--quota-remaining","80","--same-failure-count","2")
