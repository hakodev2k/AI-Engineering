...
class WatchdogTests(unittest.TestCase):
    def run_guard(self, timeout, code):
        cp = subprocess.run([sys.executable, str(SCRIPT), "--timeout", str(timeout), "--hook-id", "t", sys.executable, "-c", code], capture_output=True, text=True, timeout=max(8, timeout + 3))
        payload = json.loads(cp.stdout.strip().splitlines()[-1])
        return cp, payload

    def test_success(self):
        cp, p = self.run_guard(5, "print('ok')")
        self.assertEqual(cp.returncode, 0)
        self.assertEqual(p["status"], "success")
        self.assertIn("ok", p["stdout"])

    def test_failure(self):
        cp, p = self.run_guard(5, "raise SystemExit(7)")
